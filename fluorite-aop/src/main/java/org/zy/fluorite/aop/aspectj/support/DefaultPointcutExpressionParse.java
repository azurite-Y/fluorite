...
public class DefaultPointcutExpressionParse implements PointcutExpressionParse {
		...
		if (doubleDelimitresFlag && !wildcardCharacterFlag) {
			...
			if (targetClass.getPackage().getName().equals(packageStr)) {
				if (matcherMethod) { // ".."之后还有内容
					...
					pointcutMatcher.setPointcutMethod(method);
					return pointcutMatcher;
				}
	...
	private boolean parseMethodRank(PointcutMatcher pointcutMatcher, Class<?> targetClass , Class<?> aspectClass ) {
		...
		Method method = pointcutMatcher.getPointcutMethod();
		...
		return parseReturnType(pointcutAnnotation,method) && parseAccessModifier(pointcutAnnotation,method);
	}
