...
public final class MimeMappings implements Iterable<MimeMappings.Mapping> {
	...
	/**
	 * 从指定的映射创建一个新的mimemapes.
	 * @param mappings - 以扩展名为键、mime类型为值的源映射
	 */
	public MimeMappings(Map<String, String> mappings) {
		Assert.notNull(mappings, "Mappings不能为null");
		this.map = new LinkedHashMap<>();
	 ...
	 * @param mimeType - 要映射的mime类型
	 * @return 任何以前的映射或{@code null}
	 */
	 ...
	 * 删除现有映射.
	 * @param extension - 文件扩展名（不包括“.”）
	 * @return 删除的mime映射或{@code null}(如果没有项目被删除)
	 */
	...
	/**
	 * 为指定映射创建一个新的不可修改的视图。尝试修改返回map的方法将抛出{@link UnsupportedOperationException}
	 * @return 指定映射的不可修改视图.
	 */
	...
	public static final class Mapping {
		...
		public Mapping(String extension, String mimeType) {
			Assert.notNull(extension, "Extension不能为null");
			Assert.notNull(mimeType, "MimeType不能为null");
